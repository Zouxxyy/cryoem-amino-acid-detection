...
class FPN(nn.Module):
    ...
    def __init__(self, cf, conv, operate_stride1=False):
        ...
        if operate_stride1:
            ...
            self.C1 = conv(start_filts, start_filts, ks=7, stride=(1, 1, 1) if conv.dim == 3 else 2, pad=3, norm=cf.norm, relu=cf.relu)

        else:
            self.C1 = conv(cf.n_channels, start_filts, ks=7, stride=(1, 1, 1) if conv.dim == 3 else 2, pad=3, norm=cf.norm, relu=cf.relu)

        start_filts_exp = start_filts * self.block_expansion

        C2_layers = []
        C2_layers.append(nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
                         if conv.dim == 2 else nn.MaxPool3d(kernel_size=3, stride=(1, 1, 1), padding=1))
        ...
        if conv.dim == 2:
            ...
            self.P1_upsample = Interpolate(scale_factor=(1, 1, 1), mode='trilinear')
            self.P2_upsample = Interpolate(scale_factor=(1, 1, 1), mode='trilinear')
